...
public:
    ...
    geometry_msgs::Point getRandomConfig() {
        ...
        //todo: check for collisions. 碰撞检测
        return point;
    }

    std::map<float, Node> distance_map;

    Node getNearestNode(geometry_msgs::Point p) { //选取拓展点
    ...
    float getEuclideanDistance(geometry_msgs::Point p1, geometry_msgs::Point p2) {
        return std::sqrt(std::pow((p1.x - p2.x), 2) + std::pow((p1.y - p2.y), 2)); //距离公式
    } 
    ...
    Node expand(Node p1, Node p2, std::vector<visualization_msgs::Marker> obsVec, int frameid) {
        ...
        // calculate if the point is within an obstacle 判断随机点是否在障碍物内
        if (!intersectsObs(p1.point, p.point, obsVec) && isWithinWorld(p.point)) {
            ...
            p.parentId = p1.id;
            p1.children.push_back(p); //children of init is not in the nodeslist 初始化的子节点不在节点列表中
    ...
    bool isWithinWorld(geometry_msgs::Point p) { //判断是否在Rviz世界地图中
        return (p.x > this->x_min && p.x < this->x_max && p.y > this->y_min && p.y < this->y_max);
    }

    bool
    intersectsObs(geometry_msgs::Point p1, geometry_msgs::Point p2, std::vector<visualization_msgs::Marker> obsVec) { //提供障碍物位置信息，从而判断是否与障碍物相交
        ...
        for (int i = 0; i < obsVec.size(); i++) {
            ...
            //check for the bottom intersection 底部相交
            ...
            if (bottom || left || right || top) { 
                return true; //有一个条件符合相交，就判断为相交
            }
        }
        return false;
    }

    bool lineIntersect(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) { //判断是否相交

        // calculate the distance to intersection point 计算到相交点的距离
        float uA = ((x4 - x3) * (y1 - y3) - (y4 - y3) * (x1 - x3)) / ((y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1));
        float uB = ((x2 - x1) * (y1 - y3) - (y2 - y1) * (x1 - x3)) / ((y4 - y3) * (x2 - x1) - (x4 - x3) * (y2 - y1));

        // if uA and uB are between 0-1, lines are colliding 相交条件
        if (uA >= 0 && uA <= 1 && uB >= 0 && uB <= 1) {

            float intersectionX = x1 + (uA * (x2 - x1));
            float intersectionY = y1 + (uA * (y2 - y1)); //相交点坐标
